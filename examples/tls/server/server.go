...
import (
	...
	ntls "github.com/wubbalubbaaa/easyNet/extension/tls"
...
func main() {
	...
	isClient := false
	g.OnOpen(ntls.WrapOpen(tlsConfig, isClient, 0, func(c *easyNet.Conn, tlsConn *tls.Conn) {
		log.Println("OnOpen:", c.RemoteAddr().String())
	}))
	g.OnClose(ntls.WrapClose(func(c *easyNet.Conn, tlsConn *tls.Conn, err error) {
		log.Println("OnClose:", c.RemoteAddr().String())
	}))
	g.OnData(ntls.WrapData(func(c *easyNet.Conn, tlsConn *tls.Conn, data []byte) {
	...
	err = g.Start()
	if err != nil {
		log.Fatalf("easyNet.Start failed: %v\n", err)
		return
	}
	defer g.Stop()
